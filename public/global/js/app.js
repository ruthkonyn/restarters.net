    ...
    success: function success(response) {
      ...
      if (Object.keys($notifications).length > 0) {
        ...
        $.each($notifications, function (index, $notification) {
          $('.notification-menu-items').append($('<li>').append($('<a>').attr('href', "http://restarters.test:8000" + $notification.id).text($notification.data.title)).attr('class', 'notifcation-text'));
        });
      }
    }
  });
